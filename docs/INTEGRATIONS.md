...
- Schema: `schemas/evidence.schema.json#/$defs/contributionProvenance` references the unchanged vendored `schemas/vendor/praxis/provenance-interchange.schema.json` and `provenance-actor.schema.json` (source commit and SHA-256 in `schemas/vendor/praxis/SOURCE.json`), or accepts any object tagged with another `praxis.provenance/<major>`, which is carried verbatim. JSON Schema is only the structural floor; the full receiving rules are in `src/tutela_contribution_provenance.py`, tested against every vendored Praxis case (`tests/fixtures/praxis-provenance/`).
